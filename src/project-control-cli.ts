...
export async function runProjectControlValidation(root: URL): Promise<void> {
  if (
    ...
    throw new Error(
      "ROADMAP_UNVERIFIED: explicit v16 Owner decision record missing or inconsistent",
    );
  }
  ...
  console.log(
    `Project control validation passed: 2026.09.08-v16, MP-06 (GitHub #12), exact precedence remediation and one continuation authorized; deployment requires independent candidate/state evidence; PR requires TEST acceptance/final review; merge and Production blocked, ${warningSuffix}`,
  );
}
