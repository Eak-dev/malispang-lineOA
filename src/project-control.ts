...
function validateProjectControlInherited(
  ...
  expectEqual(
    errors,
    roadmap.version,
    v23 ? addendum.version : successor ? completion.version : "2026.09.09-v19",
...
export function evaluateProjectAction(
  ...
  if (
    ...
    if (
      ...
      return { allowed: false, reason: "V31_REVIEW_ONLY_NO_REMOTE_AUTHORITY" };
    const projected = projectV31(roadmap, currentWork);
    return evaluateProjectAction(projected.roadmap, projected.work, action);
  }
  if (
    isRecord(roadmap) &&
  ...
  if (isRecord(roadmap) && roadmap.version === PR15_CI_CONTROL.version) {
    if (
      ...
      return { allowed: false, reason: "V29_REVIEW_ONLY_NO_REMOTE_AUTHORITY" };
    const projected = projectV29(roadmap, currentWork);
    return evaluateProjectAction(projected.roadmap, projected.work, action);
  }
...
function validateSchemaDocumentsInherited(
  ...
  if (
    ...
    if (
      ...
      (!isRecord(currentWorkSchema) ||
        !isRecord(currentWorkSchema.properties) ||
        !Array.isArray(currentWorkSchema.required) ||
        !currentWorkSchema.required.includes("wp8fV24LiveUatEnablement") ||
        JSON.stringify(
          currentWorkSchema.properties.wp8fV24LiveUatEnablement,
        ) !== JSON.stringify({ const: WP8F_V24_ENABLEMENT }))
    )
    ...
    if (
      ...
      (!isRecord(currentWorkSchema) ||
        !isRecord(currentWorkSchema.properties) ||
        !Array.isArray(currentWorkSchema.required) ||
        !currentWorkSchema.required.includes("wp8fV27ProviderHangSeal") ||
        JSON.stringify(currentWorkSchema.properties.wp8fV27ProviderHangSeal) !==
...
function validateWp8fOwnerDecisionRecordInherited(
  record: unknown,
  ...
  if (version === WP8F_V27_PROVIDER_SEAL.version) {
    ...
    const current = record
      .split("## MP-OD-2026-09-20-V27 —")[1]
      ?.split("\n## ")[0];
    return (
      typeof current === "string" &&
      validateWp8fOwnerDecisionRecord(record, WP8F_V26_WORKFLOW_SEAL.version) &&
      [
        g.mandate,
        g.type,
        g.path,
        g.candidateFileSha256,
        g.baseline,
        ...[g.diagnostic, g.correction].flatMap((s) => [
          s.commit,
          s.parent,
          s.parentFileSha256,
          s.sealedFileSha256,
        ...
        "watchdog5000ms unchanged",
        g.historicalCausality,
        "Data Studio hold",
        "pendingTemplate",
        "Production NO_GO — NOT TOUCHED",
  ...
  if (version === WP8F_V25_ADDENDUM.version) {
    ...
    return (
      typeof current === "string" &&
      validateWp8fOwnerDecisionRecord(record, WP8F_V24_ENABLEMENT.version) &&
      [
        WP8F_V25_ADDENDUM.type,
        WP8F_V25_ADDENDUM.commit,
        WP8F_V25_ADDENDUM.path,
        WP8F_V25_ADDENDUM.candidateFileSha256,
        WP8F_V25_ADDENDUM.parentFileSha256,
        WP8F_V25_ADDENDUM.instrumentedFileSha256,
        WP8F_V25_ADDENDUM.pathDiffSha256,
        WP8F_V22_AUTHORIZATION.sourceCommit,
        WP8F_V22_AUTHORIZATION.artifactSha256,
        "supersedes 2026.09.11-v24",
        "no mint/reset/reissue grants",
        "complete candidate-to-HEAD inventory",
        "watchdog5000ms unchanged",
  ...
  if (version === WP8F_V24_ENABLEMENT.version) {
    const current = record
      .split("## MP-OD-2026-09-11-V24 —")[1]
      ?.split("\n## ")[0];
    return (
      typeof current === "string" &&
      validateWp8fOwnerDecisionRecord(record, WP8F_V23_ADDENDUM.version) &&
      [
        "supersedes 2026.09.11-v23",
        WP8F_V24_ENABLEMENT.type,
        WP8F_V22_AUTHORIZATION.sourceCommit,
        WP8F_V22_AUTHORIZATION.artifactSha256,
        "no mint/reset/reissue grants",
        "15000ms maximum only for the three named control tests",
        "ROOT_CAUSE_UNRESOLVED / ACCEPTED_TEST_ONLY_RESIDUAL",
        "mp06-pilot-control-v1",
...
export function evaluateWp8fPaths(
  ...
  if (
    ...
    return samePathSet(
      Array.isArray(paths) ? (paths as string[]) : [],
      PR15_P1_REMEDIATION_CONTROL.remediationPaths,
    )
      ? {
          allowed: true,
          reason: "V31_EXACT_LOCAL_P1_AND_TEST_WATCHDOG_PATHS",
        }
      : { allowed: false, reason: "UNKNOWN_OR_OUT_OF_SCOPE_PATH" };
  if (
...
const WP8F_V24_ENABLEMENT = {
  version: "2026.09.11-v24",
  ownerDecision: "MP-OD-2026-09-11-V24",
  supersedes: "2026.09.11-v23",
  type: "TEST_LIVE_UAT_ENABLEMENT_CONTROL_ONLY",
  grants: "INHERIT_EXISTING_V22_JOURNAL_NO_MINT_RESET_OR_REISSUE",
  runtimeAndArtifact: "EXACT_UNCHANGED_V22_AUTHORIZATION",
  controlTiming: {
    inspector: "inspectV23SealedRepository",
    path: "tests/project-control.test.ts",
    tests: [
    ...
    maximumWatchdogMs: 15000,
    condition: "ALL_PHASES_COMPLETE_NO_SEMANTIC_ASSERTION_FAILURE",
    semantics:
      "PRESERVE_ASSERTIONS_GIT_PROOF_ORDER_PARALLELISM_NO_RETRY_SKIP_OR_INCOMPLETE_PROOF",
    log: "FIXED_PHASE_NAME_AND_MONOTONIC_MILLISECONDS_ONLY",
    residual: "ROOT_CAUSE_UNRESOLVED / ACCEPTED_TEST_ONLY_RESIDUAL",
  },
  dataStudio: {
    account: "c395a1bc15b7c95267173de5ccd6407d",
    namespace: "a87e8a2d78b447d7a71d4130ab6b31b5",
    uniqueName: "mp06-pilot-control-v1",
    maximumLogicalObservations: 1,
    maximumTechnicalAttemptsBeforeSql: 3,
    selection: "SINGLE_EXACT_VISIBLE_NAME_VALUE_NO_OBJECT_LIST_OR_GUESSED_ID",
    query: "SELECT_ONLY_SANITIZED_SCHEMA_COUNTS_INVARIANTS_NO_OWNER_REFERENCE",
    executedSql:
      "CONSUMES_LOGICAL_OBSERVATION_NO_RETRY_ON_REJECTED_TIMEOUT_UNKNOWN",
    ownerClaims:
      "EXISTING_AUTHENTICATED_TEST_OWNER_BOUND_DIAGNOSTIC_NO_REFERENCE_EXPOSURE_OR_ENUMERATION",
  },
  credential:
    "EXISTING_TEST_ADMIN_KEY_KEYCHAIN_JUST_IN_TIME_SHORT_LIVED_MEMORY_ONLY",
  deploymentGate:
    "EXACT_FINAL_V24_CI_SUCCESS_FULL_VALIDATION_FROZEN_REPRODUCTION_COMPLETE_INVENTORY_AND_FRESH_TEST_INVARIANTS",
  grantsUsedAfterAmbiguity: "CONSUMED_NO_RETRY_OR_REPLACEMENT",
  closure: "FINAL_OWNER_DECISION_REQUIRED_PRESERVE_U1_A1_A3_BILLING_GAPS",
  production: "NO_GO_NOT_TOUCHED",
} as const;
const WP8F_V24_WORK_KEYS = [...WP8F_V23_WORK_KEYS, "wp8fV24LiveUatEnablement"];
const WP8F_V25_ADDENDUM = {
  version: "2026.09.12-v25",
  ownerDecision: "MP-OD-2026-09-12-V25",
  supersedes: "2026.09.11-v24",
  type: "SEALED_WORKER_TIMING_ONLY_ADDENDUM_INHERITING_V24",
  commit: "ce2ef9a9a75044a932d1d96ba2a8c0c633c23f4c",
  path: "worker-tests/mp-06-pilot-control.test.ts",
  candidateFileSha256:
    "4498bc3159bb496a76632f7f6908f1d9a71b4dd5b7fb594d3b863a76f5b615ae",
  parentFileSha256:
  ...
  pathDiffSha256:
    "9b1b8ba2e389609a4a76d88cddee23e4f3057fd00d2176a4c1124659d2084f37",
  additions: 31,
  removals: 0,
  test: "reproduces a provider hang through the webhook and leaves durable fail-closed checkpoints",
  watchdogMs: 5000,
  purpose:
    "MONOTONIC_TIMING_ONLY_NO_ASSERTION_FAULT_CALL_ORDER_OR_RUNTIME_CHANGE",
  residual: "ROOT_CAUSE_UNRESOLVED",
  grants: "INHERIT_EXISTING_V22_JOURNAL_NO_MINT_RESET_OR_REISSUE",
  runtimeAndArtifact: "EXACT_UNCHANGED_V22_AUTHORIZATION",
} as const;
const WP8F_V25_WORK_KEYS = [
  ...WP8F_V24_WORK_KEYS,
...
const WP8F_V27_PROVIDER_SEAL = {
  version: "2026.09.20-v27",
  ownerDecision: "MP-OD-2026-09-20-V27",
  supersedes: "2026.09.20-v26",
  type: "SEALED_PROVIDER_HANG_SYNCHRONIZATION_CONTROL_ONLY_INHERITING_V26",
  mandate: "ISSUE12_PO_REVIEW_SECTION12_OWNER_ADOPTED",
  historicalRun: "35469493106",
  test: "reproduces a provider hang through the webhook and leaves durable fail-closed checkpoints",
  path: "worker-tests/mp-06-pilot-control.test.ts",
  candidateFileSha256:
    "4498bc3159bb496a76632f7f6908f1d9a71b4dd5b7fb594d3b863a76f5b615ae",
  baseline: "eb10a187f4d37636f984d7b559ebdf22a1ddd8c1",
  diagnostic: {
    commit: "7ad01bf623e0c6b74df579e2a39b6f94040023c3",
    parent: "eb10a187f4d37636f984d7b559ebdf22a1ddd8c1",
    parentFileSha256:
      "463db843c2cd64eeada1628f23c7e73dd3aefdf8167fba885c8f7b78b085621e",
    sealedFileSha256:
      "b931ddbe9b0a1674f06ff9bcf733df804af53b3cf455b720141cc011e61cb811",
    pathDiffSha256:
      "931660fbfe0990082252c08e4a727526da5be656c868280a71a6ede989285edb",
    parentBlobOid: "d3d1b2c4fac912e0a123e38771187e71c0e1f1b7",
    sealedBlobOid: "2fb185d4ff5ce149a960b8d16bbb1dd3afaca180",
    additions: 79,
    removals: 1,
    commitPaths: [
      "docs/project/EXECUTION_GATES.md",
  ...
  correction: {
    commit: "74d893dc2a7d1db13e1c4553c4a2d1e3e3914341",
    parent: "7ad01bf623e0c6b74df579e2a39b6f94040023c3",
    parentFileSha256:
      "b931ddbe9b0a1674f06ff9bcf733df804af53b3cf455b720141cc011e61cb811",
    sealedFileSha256:
      "7c307e87e6b11ce2d9ddedefd052b97f479fec8a39e36760590116d29ca97df3",
    pathDiffSha256:
      "f8c2cc80c9b622e7e8cfc97514f2a063dab52bf848b595e0c904686ab49d8e31",
    parentBlobOid: "2fb185d4ff5ce149a960b8d16bbb1dd3afaca180",
    sealedBlobOid: "a92b10237c194e8a443269e7e88285c92d5a51cf",
    additions: 225,
    removals: 94,
    commitPaths: [
      "docs/project/EXECUTION_GATES.md",
  ...
  watchdogMs: 5000,
  purpose: "EXACT_TEST_TARGET_DEADLINE_BARRIER_AFTER_BOUNDED_CAUSAL_PROOF",
  historicalCausality:
    "HISTORICAL_RUN_CAUSALITY_NOT_ESTABLISHED_NO_GREEN_RUN_WAIVER",
  grants: "INHERIT_EXISTING_V22_JOURNAL_NO_MINT_RESET_OR_REISSUE",
  runtimeAndArtifact: "EXACT_UNCHANGED_V22_AUTHORIZATION",
...
export function validateV23SealedObservation(input: unknown): boolean {
  return validateSealedObservation(
    input,
    WP8F_V23_ADDENDUM,
    WP8F_V23_ADDENDUM.candidateFileSha256,
    WP8F_V23_ADDENDUM.instrumentedFileSha256,
...
/** Closed data validation only. Repository provenance still requires the private proof. */
export function validateV25SealedObservation(input: unknown): boolean {
  if (
    !isRecord(input) ||
    Object.keys(input).length !== 2 ||
    !exactPaths(Object.keys(input), ["previous", "current"])
  )
    return false;
  const history = [WP8F_V25_ADDENDUM.commit, WP8F_V23_ADDENDUM.commit];
  const complete = (value: unknown) =>
    isRecord(value) &&
    Array.isArray(value.paths) &&
    Array.isArray(value.historyPaths) &&
    value.paths.length === WP8F_V18_ENVELOPE.controlFiles.length + 1 &&
    value.historyPaths.length === WP8F_V18_ENVELOPE.controlFiles.length + 1;
  return (
    complete(input.previous) &&
    complete(input.current) &&
    validateSealedObservation(
      input.previous,
      WP8F_V23_ADDENDUM,
      WP8F_V23_ADDENDUM.candidateFileSha256,
      WP8F_V25_ADDENDUM.instrumentedFileSha256,
...
/** Validate both new immutable Worker steps before projecting the historical
 * v26 layer. Full inventories are never omitted; this is not a checkout proof. */
export function validateV27SealedObservation(input: unknown): boolean {
  if (
    !isRecord(input) ||
    !samePathSet(Object.keys(input), ["previous", "diagnostic", "correction"])
  )
    return false;
  const g = WP8F_V27_PROVIDER_SEAL;
  ...
  if (
    !isRecord(old) ||
    !samePathSet(Object.keys(old), ["previous", "workflow"]) ||
    !isRecord(old.previous) ||
    !samePathSet(Object.keys(old.previous), ["previous", "current"]) ||
    !isRecord(old.previous.previous) ||
    !isRecord(old.previous.current) ||
    !isRecord(old.workflow)
  )
    return false;
  const inventory = [
    ...WP8F_V18_ENVELOPE.controlFiles,
  ...
  for (const row of [
    ...
    if (
      !isRecord(row) ||
      !samePathSet(row.paths, inventory) ||
      !samePathSet(row.historyPaths, inventory)
    )
      return false;
  for (const row of [
    old.previous.previous,
    ...
    if (
      !isRecord(row) ||
      row.headFileSha256 !== g.correction.sealedFileSha256 ||
      row.workingFileSha256 !== g.correction.sealedFileSha256 ||
      JSON.stringify(row.pathCommits) !== JSON.stringify(history)
    )
      return false;
  for (const name of ["diagnostic", "correction"] as const) {
    const row = input[name],
      seal = g[name];
    if (
      !isRecord(row) ||
      !samePathSet(Object.keys(row), [
        "commit",
        "parent",
        "path",
        "candidateFileSha256",
      ...
      row.commit !== seal.commit ||
      row.parent !== seal.parent ||
      row.path !== g.path ||
      row.candidateFileSha256 !== g.candidateFileSha256 ||
      row.parentFileSha256 !== seal.parentFileSha256 ||
      row.sealedFileSha256 !== seal.sealedFileSha256 ||
      row.pathDiffSha256 !== seal.pathDiffSha256 ||
      row.parentBlobOid !== seal.parentBlobOid ||
      row.sealedBlobOid !== seal.sealedBlobOid ||
      row.headBlobOid !== g.correction.sealedBlobOid ||
      row.additions !== seal.additions ||
      row.removals !== seal.removals ||
      !samePathSet(row.commitPaths, seal.commitPaths)
    )
      return false;
  }
  const historicalWorker = (row: Record<string, unknown>) => ({
    ...row,
    headFileSha256: WP8F_V25_ADDENDUM.instrumentedFileSha256,
    workingFileSha256: WP8F_V25_ADDENDUM.instrumentedFileSha256,
    pathCommits: [WP8F_V25_ADDENDUM.commit, WP8F_V23_ADDENDUM.commit],
  });
  return validateV26SealedObservation({
    previous: {
      previous: historicalWorker(old.previous.previous),
      current: historicalWorker(old.previous.current),
    },
...
function validateSealedObservation(
  input: unknown,
  g: typeof WP8F_V23_ADDENDUM | typeof WP8F_V25_ADDENDUM,
  parentDigest: string,
  headDigest: string,
  pathCommits: readonly string[],
): boolean {
  ...
  return (
    ...
    input.parentFileSha256 === parentDigest &&
    input.sealedFileSha256 === g.instrumentedFileSha256 &&
    input.headFileSha256 === headDigest &&
    input.workingFileSha256 === headDigest &&
    ...
    JSON.stringify(input.pathCommits) === JSON.stringify(pathCommits) &&
...
export function inspectV23SealedRepository(
  ...
  const mark = (phase: string) => {
    if (!v24ControlTiming.hasSubscribers) return;
    const now = performance.now();
    v24ControlTiming.publish(
      Object.freeze({ phase, milliseconds: now - previous }),
    );
    previous = now;
  };
  try {
    ...
    mark("v23_inspect.inventory_history");
    const numstat = git(
      ...diffArgs,
      "--numstat",
      parents[1]!,
    ...
    if (v31) {
      ...
      if (
        ...
        return { ok: false, reason: "V31_SCOPE_DRIFT" };
      v31ProviderCommits = git(
        "log",
        "--full-history",
        "--format=%H",
        c.baseline + ".." + v31Head,
        ...
        .split("\n")
        .filter(Boolean);
      if (v31ProviderCommits.length > 1)
        return { ok: false, reason: "V31_PROVIDER_TEST_HISTORY_DRIFT" };
    }
    if (v30) {
      const c = PR15_CI_TIMEOUT_CONTROL;
      const v30Head = v31 ? PR15_P1_REMEDIATION_CONTROL.baseline : head;
      const v30Work = v31
        ? (JSON.parse(
            git("show", v30Head + ":config/project/current-work.json"),
          ) as Record<string, unknown>)
        : committedWork;
      if (JSON.stringify(v30Work.wp8fV30CiTimeout) !== JSON.stringify(c))
        return { ok: false, reason: "V30_EXACT_CONTROL_INVALID" };
      git("merge-base", "--is-ancestor", c.baseline, v30Head);
      const changed = list(
        git(...diffArgs, "--name-only", "-z", c.baseline, v30Head),
      );
      const history = list(
        git(
          "log",
      ...
      if (changes.length === 0) {
        if (
          hash(git("show", v30Head + ":" + c.path)) !== c.priorFileSha256 ||
          (!v31 && hash(readFileSync(join(cwd, c.path))) !== c.priorFileSha256)
        )
        ...
        if (
          ...
          hash(git("show", v30Head + ":" + c.path)) !== c.fileSha256 ||
          (!v31 && hash(readFileSync(join(cwd, c.path))) !== c.fileSha256)
        )
    ...
    if (v29) {
      ...
      const changes = git(
        ...
        .filter(Boolean);
      if (changes.length > 1)
        return { ok: false, reason: "V29_WORKFLOW_HISTORY_DRIFT" };
      if (changes.length === 1) {
        v29WorkflowCommit = changes[0]!;
        const parents = git(
          "rev-list",
          "--parents",
          "-n",
          ...
          .split(" ");
        if (parents.length !== 2)
          return { ok: false, reason: "V29_WORKFLOW_PARENT_INVALID" };
        const parent = parents[1]!;
        const authorization = JSON.parse(
          git("show", parent + ":config/project/current-work.json"),
        ) as Record<string, unknown>;
        if (
          authorization.roadmapVersion !== c.version ||
          JSON.stringify(authorization.wp8fV29PrCi) !== JSON.stringify(c) ||
          !samePathSet(
            list(
              git(...diffArgs, "--name-only", "-z", parent, v29WorkflowCommit),
            ),
            [c.path],
          ) ||
    ...
    const v31InheritedPaths = v31
      ? paths.filter(
          (path) =>
            path !== "worker/durable-objects.ts" &&
            path !== "worker-tests/durable-state.test.ts",
        )
      : paths;
    const v31InheritedHistoryPaths = v31
      ? historyPaths.filter(
          (path) =>
            path !== "worker/durable-objects.ts" &&
            path !== "worker-tests/durable-state.test.ts",
      ...
      : historyPaths;
    if (v31 && v31ProviderCommits.length === 1) {
      observation.headFileSha256 =
        WP8F_V27_PROVIDER_SEAL.correction.sealedFileSha256;
      observation.workingFileSha256 =
        WP8F_V27_PROVIDER_SEAL.correction.sealedFileSha256;
    ...
    if (v28) {
      ...
      observation.historyPaths = v31InheritedHistoryPaths.filter(
        (path) => path !== WP8F_V28_GREPTILE_REVIEWER.path,
      );
    } else if (v31) {
      observation.paths = v31InheritedPaths;
      observation.historyPaths = v31InheritedHistoryPaths;
    }
    let dirty: string[];
    if (v26) {
    ...
    if (v26 || committedWork.roadmapVersion === WP8F_V25_ADDENDUM.version) {
      ...
      git("merge-base", "--is-ancestor", g.commit, next.commit);
      git("merge-base", "--is-ancestor", next.commit, head);
      const nextParents = git("rev-list", "--parents", "-n", "1", next.commit)
        .trim()
        .split(" ");
      if (nextParents.length !== 2)
        return { ok: false, reason: "V25_SEALED_PARENT_INVALID" };
      const nextStat = git(
        ...diffArgs,
        "--numstat",
        nextParents[1]!,
      ...
      const nextObservation = {
        ...
        additions: Number(nextStat[0]),
        removals: Number(nextStat[1]),
        commitPaths: list(
          git(...diffArgs, "--name-only", "-z", nextParents[1]!, next.commit),
        ),
      };
      sealValid =
      ...
      if (v26) {
        ...
        sealValid =
          ...
          validateV26SealedObservation(inheritedObservation);
        if (v27) {
          const provider = WP8F_V27_PROVIDER_SEAL;
          git(
            "merge-base",
            "--is-ancestor",
            workflow.commit,
          ...
          const inspectStep = (
            seal: typeof provider.diagnostic | typeof provider.correction,
          ) => {
            if (
              git("rev-parse", seal.commit + "^{commit}").trim() !==
                seal.commit ||
              git("rev-list", "--parents", "-n", "1", seal.commit).trim() !==
                seal.commit + " " + seal.parent
            )
              throw new Error("V27_SEALED_COMMIT_OR_PARENT_INVALID");
            git("merge-base", "--is-ancestor", seal.commit, head);
            const stepStat = git(
              ...diffArgs,
              "--numstat",
              seal.parent,
            ...
            return {
              ...
              additions: Number(stepStat[0]),
              removals: Number(stepStat[1]),
              commitPaths: list(
              ...
              headBlobOid:
                v31 && v31ProviderCommits.length === 1
                  ...
                  : git("rev-parse", head + ":" + provider.path).trim(),
            };
          };
          sealValid =
            nextStat[2] === next.path &&
            ...
            validateV27SealedObservation({
              previous: inheritedObservation,
              diagnostic: inspectStep(provider.diagnostic),
              correction: inspectStep(provider.correction),
            });
        }
      }
    }
    ...
    if (v28) {
      ...
      const configured = v31InheritedPaths.includes(reviewer.path);
...
function validateWp8fSuccessor(
  ...
  if (
    Object.keys(work).length !== keys.length ||
    !exactPaths(Object.keys(work), keys)
  )
  ...
  if (
    v24 &&
    JSON.stringify(work.wp8fV24LiveUatEnablement) !==
      JSON.stringify(WP8F_V24_ENABLEMENT)
  )
    errors.push("V24_LIVE_UAT_ENABLEMENT_INVALID");
  if (
    v25 &&
    JSON.stringify(work.wp8fV25WorkerTimingAddendum) !==
  ...
  if (
    v27 &&
    JSON.stringify(work.wp8fV27ProviderHangSeal) !==
...
export const PR15_P1_REMEDIATION_CONTROL = {
  ...
  testTimeoutScope:
    "V26_LATER_WORKFLOW_TWO_CASES_AND_V27_PROTECTED_PATH_FIVE_CASES_ONLY",
  p1Result:
    "REPRODUCED_HTTP_DURABLE_OBJECT_SQLITE_DETERMINISTIC_BARRIER_ON_BASELINE",
  remediationPaths: [
    "tests/project-control.test.ts",
    "worker/durable-objects.ts",
    "worker-tests/durable-state.test.ts",
  ...
  recoveryContract:
    "OLDER_CLOSE_RECONCILES_WITHOUT_REMOVING_NEWER_ACTIVE_HANDOFF_AND_ACK_COMPLETES_ORIGINAL_RECEIPT",
  invariants:
    "GENERATION_FENCE_RETAINED_OPERATION_NO_RESET_NO_AUTOMATIC_RETRY_NO_NEW_CLOSE_GRANT",
  prAuthority:
    "EXISTING_PR15_DRAFT_REVIEW_ONLY_NO_READY_MERGE_OR_DUPLICATE_GREPTILE_REVIEW",
  remoteState:
...
function projectV29(
  ...
  r.ownerDecision = {
    decisionId: "MP-OD-2026-09-20-V28",
...
function validateProjectControlV29(
  ...
  if (!isRecord(roadmap) || roadmap.version !== PR15_CI_CONTROL.version)
    return validateProjectControlInherited(roadmap, work);
  if (!isRecord(work))
    return { errors: ["CURRENT_WORK_MISSING_OR_INVALID"], warnings: [] };
  const projected = projectV29(roadmap, work);
  const result = validateProjectControlInherited(
    projected.roadmap,
    projected.work,
  );
  if (
    ...
    result.errors.push("V29_EXACT_CONTROL_INVALID");
  return result;
}
...
function validateProjectControlV30(
  roadmap: unknown,
  work: unknown,
): ProjectControlValidation {
...
function projectV31(
  ...
  const r = structuredClone(roadmap),
    w = structuredClone(work);
  r.version = PR15_CI_TIMEOUT_CONTROL.version;
  r.ownerDecision = {
    decisionId: PR15_CI_TIMEOUT_CONTROL.ownerDecision,
    decidedAt: "2026-09-20",
    supersedes: PR15_CI_TIMEOUT_CONTROL.supersedes,
...
function validateProjectControlV31(
  ...
  if (
    ...
    return validateProjectControlV30(roadmap, work);
  if (!isRecord(work))
    return { errors: ["CURRENT_WORK_MISSING_OR_INVALID"], warnings: [] };
  const projected = projectV31(roadmap, work);
  const result = validateProjectControlV30(projected.roadmap, projected.work);
  if (
    work.roadmapVersion !== PR15_P1_REMEDIATION_CONTROL.version ||
...
function validateSchemaDocumentsV29(
  ...
  if (version !== PR15_CI_CONTROL.version)
    return validateSchemaDocumentsInherited(roadmapSchema, schema, version);
  if (
    !isRecord(schema) ||
    !isRecord(schema.properties) ||
    !Array.isArray(schema.required)
  )
    return ["V29_SCHEMA_NOT_CLOSED"];
  if (
    !schema.required.includes("wp8fV29PrCi") ||
    JSON.stringify(schema.properties.wp8fV29PrCi) !==
      JSON.stringify({ const: PR15_CI_CONTROL }) ||
  ...
  delete properties.wp8fV29PrCi;
  properties.roadmapVersion = { const: "2026.09.20-v28" };
  return validateSchemaDocumentsInherited(
    roadmapSchema,
    projected,
    "2026.09.20-v28",
  );
...
function validateSchemaDocumentsV30(
  roadmapSchema: unknown,
  schema: unknown,
  version = "2026.09.09-v19",
): string[] {
...
function validateSchemaDocumentsV31(
  ...
  if (version !== PR15_P1_REMEDIATION_CONTROL.version)
    return validateSchemaDocumentsV30(roadmapSchema, schema, version);
  if (
    !isRecord(schema) ||
    !isRecord(schema.properties) ||
    !Array.isArray(schema.required)
  )
    return ["V31_SCHEMA_NOT_CLOSED"];
  if (
    !schema.required.includes("wp8fV31Pr15Remediation") ||
    JSON.stringify(schema.properties.wp8fV31Pr15Remediation) !==
      JSON.stringify({ const: PR15_P1_REMEDIATION_CONTROL }) ||
    JSON.stringify(schema.properties.roadmapVersion) !==
      JSON.stringify({ const: version })
  )
    return ["V31_SCHEMA_NOT_CLOSED"];
  ...
  const properties = projected.properties as Record<string, unknown>;
  delete properties.wp8fV31Pr15Remediation;
  properties.roadmapVersion = { const: PR15_CI_TIMEOUT_CONTROL.version };
  return validateSchemaDocumentsV30(
    roadmapSchema,
    projected,
...
function validateWp8fOwnerDecisionRecordV29(
  ...
  if (version !== PR15_CI_CONTROL.version)
    return validateWp8fOwnerDecisionRecordInherited(record, version);
  if (typeof record !== "string") return false;
  const section = record
    .split("## MP-OD-2026-09-20-V29 —")[1]
    ?.split("\n## ")[0];
  return (
    typeof section === "string" &&
    Object.values(PR15_CI_CONTROL).every((value) =>
...
function validateWp8fOwnerDecisionRecordV30(
  record: unknown,
  version = "2026.09.09-v19",
): boolean {
  if (version !== PR15_CI_TIMEOUT_CONTROL.version)
...
function validateWp8fOwnerDecisionRecordV31(
  ...
  if (version !== PR15_P1_REMEDIATION_CONTROL.version)
    return validateWp8fOwnerDecisionRecordV30(record, version);
  if (typeof record !== "string") return false;
  const section = record
    .split("## MP-OD-2026-09-20-V31 —")[1]
    ?.split("\n## ")[0];
  return (
    typeof section === "string" &&
    Object.values(PR15_P1_REMEDIATION_CONTROL).every((value) =>
      section.includes(String(value)),
    ) &&
...
/** Closed PR identity, not an authorization grant or a repository proof. */
export function validatePr15MergeReceipt(
  event: unknown,
  observed: { sha: unknown; ref: unknown; merge: unknown; parents: unknown },
): { head: string; base: string } | null {
  if (
    !isRecord(event) ||
    event.number !== PR15_CI_CONTROL.pullRequest ||
    !isRecord(event.repository) ||
    event.repository.full_name !== PR15_CI_CONTROL.repository ||
  ...
  if (
    ...
    pr.head.repo.full_name !== PR15_CI_CONTROL.repository ||
    pr.base.repo.full_name !== PR15_CI_CONTROL.repository ||
    pr.head.ref !== PR15_CI_CONTROL.headBranch ||
    pr.base.ref !== PR15_CI_CONTROL.baseBranch ||
  ...
  if (
    ...
    observed.ref !== "refs/pull/15/merge" ||
