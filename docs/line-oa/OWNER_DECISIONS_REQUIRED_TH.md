...
34. **PARTIALLY RESOLVED 28 สิงหาคม 2026:** Reward Card `บัตรแต้ม TEST` ถูก Publish แยกใต้ `มะลิปัง TEST` ด้วยเป้าหมาย 50 แต้ม, Welcome bonus 0, Reminder None, cooldown วันละครั้ง และ Voucher `รางวัล TEST ไม่มีมูลค่า`; read-only configuration UAT ผ่าน แต่ต้องตัดสินใจ deviation ระหว่างข้อกำหนดเดิม 31 ธันวาคม 2026 กับค่าจริง `No expiration` ก่อนปิด Issue #3 หรือแจก URL ผ่าน Rich Menu ห้ามเชื่อม Production Reward Card
